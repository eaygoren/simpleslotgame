...
export class Symbol extends PIXI.Sprite {
    ...
    private _rowIndex: number; // Position index of the symbol in the reel

    constructor(app: any, index: number, rowIndex: number) {
        super();
    ...
    private create() {
        ...
        this.scale.set(1, 1); // Centers the anchor of the symbol for positioning
    ...
    private eventListeners() {
        this.onpointerup = () => {
            globalThis.eventBus.emit(EventNames.SymbolClicked, {
                ...
                rowIndex: this._rowIndex // Sends the row's position index
            });
        }
    }
