...
export class Payout extends PIXI.Container {
    ...
    public showPayout(data: { symbolIndex: number, rowIndex: number }) {
        if (this._isOpened) {
        ...
        this._background.position.set(PAYOUT_POSITIONS[data.rowIndex].x, PAYOUT_POSITIONS[data.rowIndex].y);
    ...
    private createPayoutValues(index: number) {
        Object.entries(SYMBOLS[index].payout).forEach(([key, value], i) => {
            ...
            payCounts.anchor.set(0, 0.5); // Center the text
            payCounts.position.set(-75, (-35 + (70 * i))); // Adjust position for each payout line
            payCounts.text = (key + "x").toString(); // Display the payout line key (e.g., "3 symbols")
            payCounts.style = {
                fontFamily: "TiltNeon",
                fontSize: 50,
                fontWeight: "700",
            ...
            payAmounts.anchor.set(1, 0.5); // Center the text
            payAmounts.position.set(75, (-35 + (70 * i))); // Adjust position for each payout line
