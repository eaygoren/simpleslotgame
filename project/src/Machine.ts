...
export class Machine extends PIXI.Container {
    ...
    private _reels: Reel;  // The slot reels
    ...
    private readonly COL_LENGTH = 1;  // Number of columns in the reels
    private readonly ROW_LENGTH = 3;  // Number of rows in the reels
    private readonly INITIAL_CREDIT: number = 100;  // Initial credit amount
    private readonly INITIAL_BET: number = BET_RANGE[0];  // Initial bet amount
    ...
    private _betIndex: number = 0;  // The index of the current bet range
    ...
    private create(col: number, row: number) {
        ...
        this._reels = new Reel(this._app, col, row);
        ...
        let interfaceBackground = new PIXI.Graphics().beginPath().roundRect(245, 590, 790, 100, 35).fill({ color: "purple", alpha: 0.25 }).closePath();
        ...
        creditLabel.position.set(-220, -20);
        ...
        this._credit.position.set(-220, 15);
        ...
        this._plusButton.position.set(275, 0);
        ...
        this._spinButton.position.set(0, 3);
        ...
        this._buttonText.position.set(0, -6);
        ...
        this._buttonText.style._stroke.width = 3;
    ...
    private eventListeners() {
        ...
        this._spinButton.onpointerup = () => {
            if (!this._reels.isSpinning) {
                ...
                globalThis.eventBus.emit(EventNames.SpinSkipped);
            }
        }
        ...
        globalThis.eventBus.on(EventNames.SymbolClicked, this.showPayout.bind(this));
    }
    //#endregion

    // Method to start the spin process
    private startSpin() {
        this._reels.startSpin();  // Start spinning
    }
